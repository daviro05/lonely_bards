...
export class BrinderComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private dialog: MatDialog, private brinderService: BrinderService) {}
    ...
    if (this.selectedCharacters.every((character) => character !== null)) {
      ...
      this.brinderService.sendMatch(matchData).subscribe(
        ...
        (error) => {
          ...
          this.openDialog('Error', 'Hubo un error al enviar la pareja. Contacta con el Centurión.');
        }
      );
    } else {
      this.openDialog('Advertencia', 'Por favor, selecciona dos personajes antes de enviar.');
    }
  }
...
onDrop(event: any, slotIndex: number) {
  ...
  const characterId = this.draggedCharacter?.id || event.dataTransfer.getData('id');
